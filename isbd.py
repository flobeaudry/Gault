...
class Isbdmsg:
    ...
    def unpack(self):
        ...
        if m[5]:                        # Only if the data is ready 
            ...
            #self.imei = ''.join([x.decode('utf-8') for x in m[5:20]])
            self.imei = ''.join([x.decode('latin1') for x in m[5:20]])
        else:
        ...
        if m[34]:
            #self.payload = ''.join(str(m[34:]))
            self.payload = (b''.join(m[34:])) # !!!
            #self.payload_hex = self.payload.hex()  # !!!
            #self.payload_hex = str(self.payload,'UTF-8')
            
            #self.payload_hex = (binascii.hexlify(self.payload)).decode()
            #self.payload = bytes(self.payload)
            
            #self.payload        = ''.join(m[34:]) # The actual message sent from the unit, bit for bit.
            #print(''.join([x.decode('latin1') for x in m[34:]]))
